...
###########################################################
# change lons from 360 to 180

def change_lons(ds): # from 0-360 to -180 - 180
    ds_ = ds
...
###########################################################
# change lons other way around

def change_lons2(ds): # from -180 - 180 to 0-360
    ds_ = ds
    ds_.coords['lon'] = ds_.coords['lon'] % 360;
    ds_new = ds_.sortby(ds_.lon)
    return ds_new

###########################################################
# plot contour lines in EOFs
...
def lon_ticks(ax, direction = 'top'):
    
    c1 = ax.set_xticks([0, 60,120,180, -120,-60], crs=ccrs.PlateCarree()); 
    c2 = ax.set_xticklabels([0, 60,120,180, -120,-60], fontsize=11);
...
###########################################################
# compute and round variance fraction into percentage
...
###########################################################
# compute sea ice volume anomalies
...
###########################################################
# histograms on sides of scatter plot
...
# select a month
def select_month(ds, i_month=1):
    ds_month = ds.isel(time=ds.groupby('time.month').groups[i_month])
    return ds_month
...
def SLP_indices(slp, mode = "ALI", norm=False):
    
    gw = compute_gridweights(slp);
    
    # Aleutian low index - SLP
    # adapted from Chen et al 2020
    # Chen et al: lat=(30,65). Captures most SLP variance + regression with Enso
    if mode == "ALI":
        ALI_   = slp.sel(lat=slice(30, 65)).sel(lon=slice(160, 220))
        ALI_gw = gw.sel(lat=slice(30, 65)).sel(lon=slice(160, 220))
        ALI    = ALI_.weighted(ALI_gw).mean("lat").mean("lon")
        ALI    = select_anomalies(ALI, clim = True, regr = True);
#         ALI    = ALI.groupby('time.month') - ALI.groupby('time.month').mean('time')
        index  = ALI;
        
    # North Pacific Oscillation index - SLP
    # Furtado et al 2011  
    if mode == "NPO":
        # note: no gridweighting now!
        npoi_np = slp.sel(lat=slice(55,72)).sel(lon=slice(180, 220)).mean("lat").mean("lon")
        npoi_np = npoi_np.groupby('time.month') - npoi_np.groupby('time.month').mean('time')
        npoi_sp = slp.sel(lat=slice(15,27)).sel(lon=slice(175, 212)).mean("lat").mean("lon")
        npoi_sp = npoi_sp.groupby('time.month') - npoi_sp.groupby('time.month').mean('time')
        NPOI    = npoi_np - npoi_sp
        index   = NPOI
...
#         NINO    = NINO.groupby('time.month') - NINO.groupby('time.month').mean('time')
    ...
    # El Nino Modoki index - SST
    # Ashok et al 2007
    if mode == "EMI":
        # note: now no gridweighting!
        SSTsel = sst.sel(lat=slice(-15,20)).sel(lon=slice(125,290))
        SSTa   = SSTsel.groupby('time.month') - SSTsel.groupby('time.month').mean('time')
        SSTa_A = SSTa.sel(lat=slice(-10,10)).sel(lon=slice(165,220)).mean("lat").mean("lon")
        SSTa_B = SSTa.sel(lat=slice(-15, 5)).sel(lon=slice(250,290)).mean("lat").mean("lon")
        SSTa_C = SSTa.sel(lat=slice(-10,20)).sel(lon=slice(125,140)).mean("lat").mean("lon")
        EMI    = SSTa_A - .5*SSTa_B - .5*SSTa_C
        index  = EMI;
        
    # North Pacific Meridional mode index - SST
    # Zhang et al 2014
    if mode == "NPMM":
        # note: now no gridweighting!
        NPMM  = sst.sel(lat=slice(20,25)).sel(lon=slice(215,225)).mean("lat").mean("lon")
        NPMM  = NPMM.groupby('time.month') - NPMM.groupby('time.month').mean('time')
        index = NPMM;
...
# calculate gridweights from ds
def compute_gridweights(ds):
    
    # have to change to:
    # use gw = np.cos(np.deg2rad(ds.lat))
    # but currently works the same (trigoniometric rules ...)
...
# shift data in time (for lead-lag function)
def lead_lag_shifts(i1, i2, lag):
    A = i1 - i2 + lag;
    if A == 0:
        nx1 = 0; nx2 = 0; ny1 = 0; ny2 = 0;
    elif A == -12:
        nx1 = 1; nx2 = 0; ny1 = 0; ny2 =-1;
    elif A == 12:
        nx1 = 0; nx2 =-1; ny1 = 1; ny2 = 0;
    
    return nx1, nx2, ny1, ny2;

###########################################################

# compute lead-lag of 2 variables
def lead_lag_corr(var1, var2, model, sim):

    monthlist = ["jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec"]

    laglist = np.arange(-6,7)
    leadlag = np.zeros([len(laglist), 12])
    leadlag_p = np.zeros([len(laglist), 12])
    Nmax = 100;

    for imonth in range(12):
        month1 = imonth + 1;

        for ilag in range(len(laglist)):
            lag = laglist[ilag]

            i1 = month1;
            i2 = np.mod(month1 + lag, 12); 
            if i2==0: i2=12;

            nx1, nx2, ny1, ny2 = lead_lag_shifts(i1, i2, lag)

            x = select_month(var1, i1)[0+nx1:Nmax+nx2]
            y = select_month(var2, i2)[0+ny1:Nmax+ny2]
        
            x["time"] = xr.DataArray(data = np.arange(len(x.time)), dims = "time")
            y["time"] = xr.DataArray(data = np.arange(len(x.time)), dims = "time")
            
#             leadlag[ilag, imonth] = np.corrcoef(x, y)[0,1]
            leadlag[ilag, imonth]   = xs.pearson_r(x, y, dim='time') # correlation
            leadlag_p[ilag, imonth] = xs.pearson_r_p_value(x, y, dim='time') # p value


    LL_corr = xr.DataArray(data = leadlag.T, dims = ["month", "lag"],  
                      coords = dict(month=(["month"], monthlist), lag=(["lag"], laglist)), 
                      attrs = dict(model = model, sim = sim))
    LL_corr = LL_corr.roll(month = 5, roll_coords = True)
    
    LL_p = xr.DataArray(data = leadlag_p.T, dims = ["month", "lag"],  
                      coords = dict(month=(["month"], monthlist), lag=(["lag"], laglist)), 
                      attrs = dict(model = model, sim = sim))
    LL_p = LL_p.roll(month = 5, roll_coords = True)

    return LL_corr, LL_p

###########################################################

# create 3d map of 1d variable
def create_index_map(index, reference):
    
    zeros      = xr.DataArray(np.zeros((reference.time.size, reference.lat.size, reference.lon.size)),
                              dims=reference.dims, coords=reference.coords);
    index_map  = xr.DataArray(np.zeros((reference.time.size, reference.lat.size, reference.lon.size)),
                              dims=reference.dims, coords=reference.coords);
    
    for i in range(reference.time.size):
        ref = (1 + zeros[i,:,:]);
        index_map[i,:,:] = index[i] * ref;
        
    return index_map

###########################################################

def select_winter(ds, mode="DJF"):
    
    Ntime = int(len(ds.time)/12);
    alt_time = xr.DataArray(data = np.arange(0, Ntime - 1), dims="time");
    
    dec = ds.isel(time = ds.groupby("time.month").groups[12]).isel(time = slice(0, Ntime-1))
    jan = ds.isel(time = ds.groupby("time.month").groups[1]).isel(time = slice(1, Ntime))
    feb = ds.isel(time = ds.groupby("time.month").groups[2]).isel(time = slice(1, Ntime))
    mar = ds.isel(time = ds.groupby("time.month").groups[3]).isel(time = slice(1, Ntime))
    apr = ds.isel(time = ds.groupby("time.month").groups[4]).isel(time = slice(1, Ntime))
    dec["time"] = alt_time; jan["time"] = alt_time; feb["time"] = alt_time; mar["time"] = alt_time; apr["time"] = alt_time;
    
    if mode == "DJF":
        ds_winter = (dec + jan + feb)/3;
    elif mode == "DJFM":
        ds_winter = (dec + jan + feb + mar)/4;
    elif mode == "FMA":
        ds_winter = (feb + mar + apr)/3;
        
    return ds_winter

###########################################################

def select_anomalies(ds, clim = True, regr = True):

    # remove climatology
    if clim == True:
        ds_a = ds.groupby('time.month') - ds.groupby('time.month').mean('time')
    else:
        ds_a = ds

    # remove linear regression
    if regr == True:
        polyfit = ds_a.polyfit(dim = "time", deg = 1)
        pred = xr.polyval(ds_a.time, polyfit)
        pred["slpa"] = pred.polyfit_coefficients

        ds_a = ds_a - pred.slpa
        ds_a = ds_a.rename("slp")
        ds_a.attrs = ds.attrs
...
def compute_regression(ds1, ds2, dim = "time"):
    
        corr_ = xs.pearson_r(ds1, ds2, dim = dim)
        pval_ = xs.pearson_r_p_value(ds1, ds2, dim = dim)
        regr_ = xs.linslope(ds1, ds2, dim = dim)
        
        output = corr_.to_dataset(name = "corr")
        output["pval"] = pval_
        output["regr"] = regr_
